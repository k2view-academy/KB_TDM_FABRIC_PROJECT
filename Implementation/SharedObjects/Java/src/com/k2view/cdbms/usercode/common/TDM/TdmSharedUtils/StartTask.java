...
public class StartTask {
	...
	public static Object perform(Long taskId, Boolean forced, Map<OverrideParamKey, Object> inputOverrides)
			throws Exception {
		return perform(taskId, forced, inputOverrides, null, null, null);
	}

	public static Object perform(Long taskId, Boolean forced, Map<OverrideParamKey, Object> inputOverrides,
			Long draftExecutionId) throws Exception {
		return perform(taskId, forced, inputOverrides, draftExecutionId, null, null);
	}

	public static Object perform(Long taskId, Boolean forced, Map<OverrideParamKey, Object> inputOverrides,
			Long draftExecutionId, String execUserName, String execUserRole) throws Exception {
		try {
			StartTaskValidator.checkSystemHealth();
			Db.Row taskRow = fetchTaskData(taskId);
			StartTaskValidator.validateInputOverrides(taskRow, inputOverrides);
			...
			if (Boolean.TRUE.equals(context.get("envOverridden")))
				overrideParams.put("IMPLICIT_OVERRIDE_LOGICAL_UNITS", context.get("logicalUnits"));

			StartTaskValidator.validateBeforeRun(taskRow, context, inputOverrides, overrideParams);
			StartTaskValidator.verifyUserPermissions(taskId, taskRow, context, overrideParams, execUserName);
			...
			Long taskExecutionId = initiateExecution(taskId, taskRow, context, overrideParams, inputOverrides,
					draftExecutionId, execUserName, execUserRole);
			...
			return wrapWebServiceResults("FAILED", "Execution Error", e.getMessage());
		}
	}
	...
	private static Map<String, Object> resolveTaskContext(Db.Row taskRow, Map<OverrideParamKey, Object> inputOverrides,
		...
		if ("RESERVE".equalsIgnoreCase(taskType) || "DELETE".equalsIgnoreCase(taskType)) {
			srcEnv = tarEnv;
		}
	...
	private static void resolveSelectionAndEntities(Db.Row taskRow, Map<OverrideParamKey, Object> inputOverrides,
		...
		Integer numEntitiesOverride = inputOverrides.get(OverrideParamKey.NO_OF_ENTITIES) != null
				? ((Long) inputOverrides.get(OverrideParamKey.NO_OF_ENTITIES)).intValue()
				: null;
		...
		if (entitiesList != null) {
			if (overrideMethod == null) {
				...
				throw new Exception("Entities list provided but method is not 'L'.");
			}
		}
		...
		if (overrideMethod != null) {
			switch (finalMethod.toUpperCase()) {
				case "L":
					if (entitiesList == null)
						throw new Exception("Entities list must be provided for 'L' selection method override.");
				...
				case "ALL":
					if (numEntitiesOverride != null && numEntitiesOverride != -1)
						throw new Exception("Number of entities must be 'all' for ALL selection.");
					finalCount = -1;
					break;
				case "C":
				case "R":
					if (numEntitiesOverride == null)
						throw new Exception("Number of entities must be provided for '" + finalMethod
								+ "' selection method override.");
					finalCount = numEntitiesOverride;
					break;
				default:
					throw new Exception("Invalid selection method '" + finalMethod
							+ "' provided for override. Only L, C, R and ALL are allowed.");
			}
		} else {
			if ("L".equalsIgnoreCase(finalMethod)) {
				String list = "" + taskRow.get("selection_param_value");
				if (list == null || "null".equals(list))
					throw new Exception("Selection method is 'L', but no entity list is defined.");
	...
	private static void handleSpecialParams(String method, Db.Row taskRow,
		...
		if ("C".equals(m)) {
			...
			if (effectiveFlow == null || effectiveFlow.isBlank() || "null".equals(effectiveFlow))
				throw new TdmValidationException("Custom Logic", "Custom logic flow name must be provided.");
			...
			if (effectiveLuName == null || effectiveLuName.isBlank() || "null".equals(effectiveLuName))
				throw new TdmValidationException("Custom Logic", "Custom logic LU name must be provided.");
	...
	private static void resolveTaskScope(Db.Row taskRow, Map<OverrideParamKey, Object> inputOverrides,
		...
		if (beId != null) {
			if (taskRow.get("be_id") != null && !"null".equals("" + taskRow.get("be_id")))
				throw new Exception("BE cannot be overridden.");
			if (lus == null) {
				...
				if ("FAILED".equals(r.get("errorCode")))
					throw new Exception("Failed to fetch logical units for BE " + beId + ": " + r.get("message"));
				lus = (List<Map<String, Object>>) r.get("result");
			}
			if (lus == null || lus.isEmpty())
				throw new Exception("Logical units cannot be empty.");
		...
		if (beId == null && lus != null) {
			...
			boolean srcChanged = newSrcEnvId != null && !newSrcEnvId.equals(toLong(taskRow.get("source_environment_id")));
			boolean tarChanged = newTarEnvId != null && !newTarEnvId.equals(toLong(taskRow.get("environment_id")));
		...
		if (lus == null || lus.isEmpty()) {
			...
			Long newSrcEnvId  = (Long) context.get("srcEnvId");
			Long newTarEnvId  = (Long) context.get("tarEnvId");
			boolean srcChanged = newSrcEnvId != null && !newSrcEnvId.equals(origSrcEnvId);
			boolean tarChanged = newTarEnvId != null && !newTarEnvId.equals(origTarEnvId);
			...
			if (beId == null && (srcChanged || tarChanged) && finalBeId != null && !"null".equals(finalBeId)) {
				...
				String tt = ("" + taskRow.get("task_type")).toUpperCase();
				boolean isSourceTask = tt.equals("EXTRACT") || tt.equals("GENERATE")
						|| tt.equals("TRAINING") || tt.equals("AI_GENERATED");
				...
				if ("FAILED".equals(envResult.get("errorCode")))
					throw new Exception("Failed to fetch LUs for override environment: " + envResult.get("message"));
				...
				if (lus == null || lus.isEmpty())
					throw new Exception("No logical units are available in the override environment");
	...
	private static Long initiateExecution(Long taskId, Db.Row taskRow, Map<String, Object> context,
		...
		if (luOverrideList != null && !luOverrideList.isEmpty()) {
			...
			for (Map<String, Object> execution : executions) {
				...
				if (auditEntry.size() > 1) // has fields beyond lu_id
		...
		if (taskExecId == null)
			throw new Exception("Failed to generate task execution ID.");
		...
		if ((executions.get(0).get("selection_method") != null
			...
			fnSaveRefExeTablestoTask((Long) executions.get(0).get("task_id"), taskExecId);
		}
		...
		fnCreateSummaryRecord(executions.get(0), taskExecId, (String) inputOverrides.get(OverrideParamKey.BE_ID),
				(String) context.get("sourceEnvName"), (Long) context.get("tarEnvId"), (Long) context.get("srcEnvId"),
				(String) context.get("targetEnvName"), execUserName, execUserRole);
	...
	private static Long fetchEnvId(String name) throws Exception {
		Long id = (Long) db(TDM)
				...
				.firstValue();
		if (id == null)
			throw new Exception("Environment '" + name + "' not found or is not active.");
		return id;
	}
