	...
	private static Map<String,String> executeExtractBatch(Map<String, Object> taskProperties) throws Exception {
		try {
			...
			if("L".equalsIgnoreCase(selectionMethod) && !isChildLU(taskProperties)){
				batchCommand = "BATCH " + luName + ".(?) fabric_command=\"sync_instance " + luName + ".?\" with " + affinity + " async=true";
			} else {
				batchCommand = "BATCH " + luName + " FROM " + entityInclusionInterface + " USING(?) fabric_command=\"sync_instance " + luName + ".?\" with " + affinity + " async=true";
			}
			...
			String batchID = (String) fabric().fetch(batchCommand, entityInclusionOverride).firstValue();
