    ...
    public static void fnTdmTaskScheduler() throws IOException, SQLException {
        ...
        db(TDM).fetch(activeTasksQuery).forEach(row -> {
            ...
            if(executionTime.isMatch(now) || (timeToNextExecution.toMinutes() == 0 && timeToNextExecution.getSeconds() <= 10)){
                ...
                Util.rte(() -> db(TDM).fetch(productQuery, taskID, environmentID, sourceEnvName).forEach(r ->
                {
                    ...
                    Util.rte(() ->
                            db(TDM).execute(insertToTaskExecutionQuery,
                                    taskID,
                                    taskExecutionID,
                                    beID,
                                    environmentID,
                ...
                // insert to summary
                Util.rte(() -> db(TDM).execute(insertToSummaryQuery, taskID, taskExecutionID, 0, 0, 0, 0, 0, 0, environmentID, taskType, beID));
            }
        });
    }
}
